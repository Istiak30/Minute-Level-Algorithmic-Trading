...
class PairInput:
    """Describe one canonical FX parquet input.

    Attributes:
        pair: FX pair code, for example EURUSD.
        dataset_tag: Filename suffix after the first underscore.
        parquet_path: Canonical parquet location.
    """
...
def fraction_0_1(value: str) -> float:
    """Parse a float in the interval (0, 1]."""
    parsed = float(value)
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for the EDA runner.
...
def parse_pair_input(path: Path) -> PairInput:
    """Parse pair metadata from a canonical parquet filename.

    Expected filename pattern:
        <PAIR>_<DATASET_TAG>.parquet

    Example:
        EURUSD_2015_2025.parquet

    Args:
        path: Canonical parquet file path.

    Returns:
        Parsed pair input descriptor.

    Raises:
        ValueError: If the filename does not match the expected structure.
    """
    stem = path.stem
    if "_" not in stem:
        raise ValueError(
            f"Parquet filename must follow '<PAIR>_<DATASET_TAG>.parquet': {path.name}"
        )
...
def discover_pair_inputs(
    ...
    """Discover canonical parquet inputs from the parquet directory.

    Args:
        parquet_dir: Directory containing canonical parquet files.
        selected_pairs: Optional pair filter.

    Returns:
        Sorted list of pair input descriptors.

    Raises:
        FileNotFoundError: If the parquet directory is missing.
        ValueError: If no matching parquet files are found.
    """
...
def snapshot_path_for(pair_input: PairInput) -> Path:
    """Build the raw snapshot path for a pair."""
...
def sample_path_for(pair_input: PairInput) -> Path:
    """Build the sample parquet path for a pair."""
...
def load_pair_parquet(pair_input: PairInput) -> pd.DataFrame:
    """Load one canonical pair parquet with validation.

    Args:
        pair_input: Canonical pair descriptor.

    Returns:
        Validated and time-sorted DataFrame.
...
def build_snapshot(pair_input: PairInput, df: pd.DataFrame, sample_frac: float, force: bool) -> Path:
    """Create immutable raw snapshot and sample parquet.

    Args:
        pair_input: Canonical pair descriptor.
        df: Canonical pair data already loaded into memory.
        sample_frac: Fraction for sample parquet creation.
        force: Whether to overwrite existing outputs.

    Returns:
        Snapshot parquet path.
    """
...
def add_eda_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Add derived in-memory EDA columns.

    Args:
        df: Input market data.

    Returns:
        DataFrame with derived return and time-based columns.
    """
    out = df.copy()
...
def compute_overview(df: pd.DataFrame) -> pd.DataFrame:
    """Compute overview sanity metrics.

    Args:
        df: Canonical pair data.
...
def compute_counts_by_year_month(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        Table of row counts by year and month.
    """
    return (
...
def compute_daily_coverage(df: pd.DataFrame) -> pd.DataFrame:
    """Compute daily observed minute counts and coverage ratios.

    Args:
        df: EDA-enriched DataFrame containing a date column.
...
def compute_monthly_coverage(daily_df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate daily coverage into monthly summary.

    Args:
        daily_df: Daily coverage table.
...
def compute_session_stats(df: pd.DataFrame) -> pd.DataFrame:
    """Compute per-session EDA metrics.

    Args:
        df: EDA-enriched pair DataFrame.
...
def compute_hourly_stats(df: pd.DataFrame) -> pd.DataFrame:
    """Compute per-hour EDA metrics.

    Args:
        df: EDA-enriched pair DataFrame.

    Returns:
        Hour-level summary table.
    """
    return (
...
def compute_returns_summary(df: pd.DataFrame) -> pd.DataFrame:
    """Compute descriptive statistics for return and range features.

    Args:
        df: EDA-enriched pair DataFrame.

    Returns:
        Summary table across core derived metrics.
    """
    rows = []
...
def simple_acf(series: pd.Series, nlags: int) -> pd.DataFrame:
    """Compute simple autocorrelation values.
    ...
    Returns:
        DataFrame with lag and autocorrelation values.
    """
...
def build_quality_summary(df: pd.DataFrame, eda_df: pd.DataFrame, daily_df: pd.DataFrame) -> pd.DataFrame:
    """Build a rule-oriented data quality summary.

    Args:
        df: Canonical pair data.
        eda_df: EDA-enriched pair data.
        daily_df: Daily coverage table.

    Returns:
        One-row quality summary table.
    """
    invalid_ohlc = (
        (df["high"] < df["low"]) |
        (df["open"] > df["high"]) |
...
def plot_line(
    ...
    """Plot and save a generic line chart."""
...
def plot_bar(x, y, title: str, xlabel: str, ylabel: str, path: Path, dpi: int) -> None:
    """Plot and save a generic bar chart."""
...
def run_pair_eda(pair_input: PairInput, df: pd.DataFrame, dpi: int, force: bool) -> None:
    """Generate all pair-level EDA artifacts.

    Args:
        pair_input: Canonical pair descriptor.
        df: Pair data loaded from the canonical parquet.
        dpi: Plot resolution.
        force: Whether to overwrite existing outputs.
    """
    ...
    plot_specs = [
        ...
        (
            ...
            lambda: plot_bar(
                ...
                "Session",
                "Average absolute 1-minute return",
                report_dir / "E2_session_volatility.png",
                dpi,
            ),
        ),
...
def run_global_eda(pair_inputs: list[PairInput], dpi: int, force: bool) -> None:
    """Generate cross-pair global EDA artifacts.

    Args:
        pair_inputs: Successfully processed pair descriptors.
        dpi: Plot resolution.
        force: Whether to overwrite existing outputs.
    """
    if not pair_inputs:
