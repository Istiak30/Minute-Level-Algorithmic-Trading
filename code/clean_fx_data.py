...
def compute_daily_counts(df: pd.DataFrame) -> pd.DataFrame:
    """Compute daily observation counts."""
    temp = df.copy()
...
def build_cleaning_summary(
    ...
    """Build a one-row cleaning summary."""
...
def clean_pair(df: pd.DataFrame, min_obs_day: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Apply deterministic cleaning rules and return cleaned data plus dropped days."""
    out = df.copy()
...
def process_pair(pair: str, min_obs_day: int, force: bool) -> None:
    """Load, clean, and save artifacts for one pair."""
...
def main() -> None:
    """Run full all-pair FX cleaning pipeline."""
    args = parse_args()
