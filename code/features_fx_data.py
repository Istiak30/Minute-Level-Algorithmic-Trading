...
def load_clean_pair(pair: str) -> pd.DataFrame:
    """Load one cleaned pair parquet with validation."""
...
def add_volatility_features(df: pd.DataFrame) -> pd.DataFrame:
    """Add rolling volatility features from 1-step returns."""
    out = df.copy()
...
def add_trend_features(df: pd.DataFrame) -> pd.DataFrame:
    """Add moving-average and momentum features."""
    out = df.copy()
...
def add_volatility_regime_features(df: pd.DataFrame) -> pd.DataFrame:
    """Add volatility regime flags using rolling quantile thresholding."""
    out = df.copy()
...
def build_cross_pair_return_map(cleaned_frames: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    """Build pair-wise cross return features for later joining."""
...
def add_cross_pair_features(
    ...
    """Join other-pair return signals into one pair dataset."""
    out = df.copy()
...
def process_pair(
    ...
    """Build and save feature artifacts for one pair."""
...
def main() -> None:
    """Run the full all-pair feature engineering pipeline."""
    args = parse_args()
