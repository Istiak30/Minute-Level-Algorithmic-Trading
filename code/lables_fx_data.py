...
def load_feature_pair(pair: str) -> pd.DataFrame:
    """Load one feature parquet with validation."""
...
def add_label_columns(
    ...
    """Add forward returns and 3-class labels."""
    out = df.copy()
...
def build_label_summary(
    ...
    """Build a one-row summary of label generation results."""
    label_primary_col = f"label_3class_{horizon_primary}"
    label_secondary_col = f"label_3class_{horizon_secondary}"
...
def build_label_distribution_table(df: pd.DataFrame, horizon: int) -> pd.DataFrame:
    """Build a label distribution table for one horizon."""
...
def build_session_label_distribution(df: pd.DataFrame, horizon: int) -> pd.DataFrame:
    """Build label distribution by session for one horizon."""
    label_col = f"label_3class_{horizon}"
    out = (
...
def process_pair(
    ...
    """Build and save labeled artifacts for one pair."""
...
def main() -> None:
    """Run the full all-pair FX label engineering pipeline."""
    args = parse_args()
